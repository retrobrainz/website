...
export default function TitleTranslatePage() {
  ...
  const translation = title?.translations?.find((tr: any) => tr.locale === locale);

  const handleSave = async (values: any) => {
    try {
      if (translation) {
        await xior.put(`/titles/${titleId}/translations/${translation.id}`, {
          ...values,
          locale,
        });
      } else {
        await xior.post(`/titles/${titleId}/translations`, {
          ...values,
          locale,
        });
      }
    ...
    <Container maxWidth="lg" style={{ paddingTop: 24 }}>
      <Breadcrumb
        items={[
          ...
          {
            title: (
              <Link href={`/titles/${title?.id}`}>{translation?.name || title?.name || '...'}</Link>
            ),
          },
          { title: t('translate') },
        ]}
      ...
      <Flex justify="space-between" align="center" style={{ marginBottom: 16 }}>
        <Typography.Title level={1} style={{ margin: 0 }}>
          {t('translate')}: {translation?.name || title?.name || '...'}
        </Typography.Title>
        <Select
          value={locale}
          onChange={setLocale}
        ...
        <Row gutter={24}>
          <Col span={12}>
            <Form.Item label={t('name')}>
              <Input value={title?.name} disabled />
            </Form.Item>
          </Col>
          <Col span={12}>
            <Form.Item
              ...
              name="name"
              rules={[{ required: true }]}
              extra={<AskGoogle query={`translate name of game "${title?.name}" into ${locale}`} />}
            >
              <Input />
            </Form.Item>
          </Col>
